...
def draw_line_path ( mx, my, from_ = 'top' ):
    ...
    if z.orientation == 'A':
        if from_ == 'top':
            ...
            z.draw( ctx, 'tl' )
            z.tl_painted = True
            draw_line_path( mx - 1, my, 'right' )
        if from_ == 'right':
            if z.br_painted == True:
                return
            z.draw( ctx, 'br' )
            z.br_painted = True
            draw_line_path( mx, my + 1, 'top' )
        if from_ == 'bottom':
            if z.br_painted == True:
                return
            z.draw( ctx, 'br' )
            z.br_painted = True
            draw_line_path( mx+1, my, 'left' )
        if from_ == 'left':
            if z.tl_painted == True:
                return
            z.draw( ctx, 'tl' )
            z.tl_painted = True
            draw_line_path( mx, my - 1, 'bottom' )
...
sw = max(1.5, random.random() * sector_wh)
ctx.set_line_width( sw )
...
while painted_sectors / 4 < num_zircles ** 2 * 0.20 :
    ...
    draw_line_path ( ini_x, ini_y, 'top' )
    draw_line_path ( ini_x, ini_y -1, 'bottom' )

    c1.rotateHue( -5 )
    c1.setLum( c1.l * 0.99 )
# draw_line_path ( ini_x, ini_y, 'left' )
