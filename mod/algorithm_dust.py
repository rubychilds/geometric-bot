...
def render( tries = 0 ):
    ...
    ctx_final.set_source_rgb(0.9882352941176471, 0.9882352941176471, 0.9411764705882353)
    ctx_final.fill ()
    ...
    gp['params']['time'] = datetime.datetime.now().strftime("%Y%m%d-%H%M%S%f")

    footline = name.footline( gp )
    draw.footline( ctx_final, footline)
    filename = name.filename( gp )

    return ims_final, filename, footline
